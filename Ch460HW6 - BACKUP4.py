...
"""
# Need numpy for ln function, exponential function, pi, and array functionality
import numpy as np
...
# Takes inputs of Temperature (T), Molecular mass (M), and electronic degeneracy (We1)
...
# Input Temperature (T) in K, Molecular mass (M) in amu, Vibrational Temperature (θv),
...
T = [800+273.15,900+273.15,1000+273.15,1100+273.15,1200+273.15]
...
for i in range(0, len(T)):
    ...
    print("At",T[i], "K , Kp = ", Kp_iodine / Pa2atm)
    
print("For the reaction I2 <-> 2I in 1/atm \n")

# Iodine reaction equilibrium constants from Perlman and Rollefson, J.Chem. Phys., 9
Texp = [872, 973, 1073, 1173, 1274]
Kexp = [[2.11E-4, 1.62E-4, 1.81E-4, 1.86E-4, 1.66E-4], 
        [1.805E-3, 1.785E-3, 1.855E-3, 1.852E-3, 1.709E-3], 
        [1.110E-2, 1.065E-2, 1.082E-2, 1.108E-2, 1.084E-2], 
        [4.874E-2, 4.769E-2, 4.798E-2, 4.836E-2, 4.773E-2],
        [0.1692, 0.1657, 0.1671, 0.1692]]
# Print out average Kp from experimental data
print("From the experimental data in Perlman and Rollefson, J.Chem. Phys., 9, p. 362")
for i in range(0,len(Texp)): print("At ",Texp[i],"K , Kp = ",np.average(Kexp[i])) 
print("For the reaction I2 <-> 2I in 1/atm \n") 
...
# Calculate the enthalpy of reaction for the reaction H2 + I2 <-> 2HI: Convert to cal/mol
print("At",np.average(T), "K , ΔH = ", ΔH_Reaction(T[0], T[1], Kc_HI[0], Kc_HI[1])/4.184)
...
for i in range(0, len(T)):
    ...
    # Calculate Kp at T. Input partitions functions and stoichemetry coefficients
    Kp_nitric = Kp_Const(T[i], qN2, qO2, qNO, 1, 0.5, 0.5, 1, 0)
    print("At",T[i], "K , Kp = ", Kp_nitric)
